...
impl SsTable {
    ...
    pub fn open(id: usize, block_cache: Option<Arc<BlockCache>>, file: FileObject) -> Result<Self> {
        ...
        let bloom_meta_offset = (&raw_bloom_offset[..]).get_u32() as u64;
        let raw_bloom = file.read(bloom_meta_offset, len - 4 - bloom_meta_offset)?;
        let bloom = Bloom::decode(&raw_bloom)?;

        let raw_meta_offset = file.read(bloom_meta_offset - 4, 4)?;
        let block_meta_offset = (&raw_meta_offset[..]).get_u32() as u64;
        let raw_meta = file.read(block_meta_offset, bloom_meta_offset - 4 - block_meta_offset)?;
